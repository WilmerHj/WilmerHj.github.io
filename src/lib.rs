...
pub fn solve_constant_coeffs(
    ...
    let bc_left = parse_bc(left_kind, left_value)?;
    let bc_right = parse_bc(right_kind, right_value)?;

    let (x, u) = fem1d_flux_form(
        x_left, x_right, nel,
...
fn fem1d_flux_form(
    ...
    let mut k = vec![0.0f64; nd*nd];
    let mut fglob = vec![0.0f64; nd];

    let xi = [ -1.0f64 / 3.0f64.sqrt(),  1.0f64 / 3.0f64.sqrt() ];
    let w  = [ 1.0f64, 1.0f64 ];
    ...
    for e in 0..nel {
        ...
        let x1 = x[n1];
        let x2 = x[n2];
        let (ke, fe) = element_p1(x1, x2, &xi, &w, c, alpha, beta, a, gamma, f);
        add22(&mut k, nd, n1, n2, &ke);
        fglob[n1] += fe[0];
        fglob[n2] += fe[1];
    }
