\begin{table}[H]
    \centering
    \caption{Cost and efficiency ratio, stress\_members}
    \label{tab:cost_efficiency_ratio_summary}
    \begin{tabular}{lrrrrrr}
        \toprule
        \textbf{Model} & \textbf{$C$} & \textbf{$\eta_{V,NASA}$} & \textbf{$\eta_{H,NASA}$} & \textbf{$\eta_{V,T43A}$} & \textbf{$\eta_{H,T43A}$} & \textbf{$\bar{\eta}$} \\
        \midrule
        Solid & 100.0 & 1.000 & 1.000 & 1.000 & 1.000 & 1.000 \\
        Beam & 88.1 & 0.881 & 1.053 & 0.919 & 0.754 & 0.902 \\
        CBUSH & 60.5 & 0.758 & 1.019 & 0.894 & 1.093 & 0.941 \\
        Bonded & 54.5 & 0.871 & 0.649 & 0.162 & 0.384 & 0.516 \\
        \bottomrule
    \end{tabular}
\end{table}
