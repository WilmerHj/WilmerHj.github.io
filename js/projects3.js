...
=
{\\omega}\\times
\\left({\\omega}\\times\\mathbf r\\right).
$$
...
The centripetal acceleration follows from a second cross product:
...
\\end{vmatrix}
=
-\\omega_z^2R\\hat{\\mathbf r}.
$$

The negative radial direction shows that the acceleration points inward toward the rotor hub. At the blade tip,

$$
\\left|\\mathbf a_{tip}\\right|
=
\\omega_z^2R
=
2.22^2\\times44.2
...
$$
R_{CG}=14.087\\ \\mathrm{m}.
$$

Its centripetal acceleration is therefore

$$
\\left|\\mathbf a_{CG}\\right|
=
\\omega_z^2R_{CG}
=
2.22^2\\times14.087
...
\\\\
&=
1.538\\times10^6\\ \\mathrm{N}
=
1.538\\ \\mathrm{MN}.
\\end{aligned}
$$

Mechanical reported a radial root reaction of 1.5388 MN, giving a difference of only **0.03%** from the independent matrix-based calculation.

## CFD verification

The aerodynamic solution was assessed using four independent checks:

* The final momentum residuals were of order 10^-6 and continuity reached 3.0 x 10^-5.
* Integrated blade pressure drifted by only **0.025%** over the final 50 iterations.
* The global mass-flow imbalance was **1.09 x 10^-6 kg/s** on a throughput of approximately 886,000 kg/s.
...
* The surface mesh did not scale with the narrowing chord. The tip retained only about a dozen faces around each section, so the pressure difference responsible for tangential force became noisy.
...
The 22.1 tonne idealized shell generated **1.54 MN of centrifugal root force**, approximately nineteen times the aerodynamic thrust on one blade. This load was checked directly from mass, angular speed and center-of-gravity radius; the hand calculation and Mechanical reaction differed by only **0.03%**.
...
The rotational and tangential components closed well, but approximately one quarter of the axial load was missing after the CFD-to-FEA transfer. Surface coverage differed by only 1.2%, so the most likely cause is non-conservative pressure interpolation flattening the leading-edge suction peak. Inconsistent shell normals are a secondary possibility that should be checked in the imported-load mapping summary.
